target("RayTracing")
    ...
    add_deps("DSMEngine")
    add_files("main.cpp", "RestirDI/**.cpp")
    add_headerfiles("TestTriangle.h", "RestirDI/**.h")

    after_build(function(target)
        local projectRoot = os.projectdir()
        local targetDir = target:targetdir()
        local restirDirectory = path.join(target:scriptdir(), "RestirDI")
        local engineShaders = path.join(projectRoot, "DSMEngine", "Shaders")
        local sampleAssets = path.join(projectRoot, "Samples", "Assets")

        os.cp(path.join(restirDirectory, "Shaders"), targetDir)
        os.cp(path.join(restirDirectory, "imgui.ini"), targetDir)
        ...
        assert(dxcRuntimeDir, "RayTracing 需要 Windows SDK 中的 dxcompiler.dll 和 dxil.dll")
