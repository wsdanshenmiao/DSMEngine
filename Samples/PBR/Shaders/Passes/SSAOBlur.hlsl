...
void StoreToCache(uint index, float2 uv)
{
    float ao = gBlurTexture.SampleLevel(gPointBorderSampler, uv, 0);
...
void SSAOBlurCS(int3 groupThreadID : SV_GroupThreadID, int3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    uv *= invTexelSize;
    StoreToCache(groupThreadID.x + blurRadius, uv);
    ...
    // 进行模糊
    float ssao = centerAO * gBlurWeights[blurRadius];
    float weightSum = gBlurWeights[blurRadius];
    for (int i = 0; i <= blurRadius * 2; ++i) {
        ...
        if(dot(centerNormal, normal) > 0.8 && abs(centerDepth - depth) < 0.2){
            ssao += neighborVal.x * gBlurWeights[i];
            weightSum += gBlurWeights[i];
        }
    }
    ssao /= weightSum;
