...
public:
    ...
    void Initialize(DSM::Renderer& renderer)
    {
        ...
        std::shared_ptr<GameObject> lihuazouObj = globalContext.world->GetObject(
            globalContext.world->CreateObject("Lihuazou")).lock();
        lihuazouObj->AddComponent<Model>(*lihuazou);
        std::shared_ptr<GameObject> planeObj = globalContext.world->GetObject(
            globalContext.world->CreateObject("Plane")).lock();
        planeObj->AddComponent<Model>(*plane);
        std::shared_ptr<GameObject> sponzaObj = globalContext.world->GetObject(
            globalContext.world->CreateObject("Sponza")).lock();
