...
namespace DSM {
    ...
    public:
        ...
        {
            static auto scene = DSMEngine::sm_GlobalContext.scene.get();
            if(scene != DSMEngine::sm_GlobalContext.scene.get()){
                scene = DSMEngine::sm_GlobalContext.scene.get();
                CreateTestScene();
            }
    ...
    private:
        ...
        void CreateTestScene()
        {
            ...
            auto processModel = [scene](std::shared_ptr<GameObject> obj, const std::shared_ptr<Model>& model) {
                ...
                for (const auto& [index, mesh] : model->meshes | std::views::enumerate) {
                    auto subObj = scene->CreateObject(mesh->GetName());
                    auto subObjPtr = scene->GetObjectByID(subObj).lock();
                    auto meshRenderer = subObjPtr->AddComponent<MeshRenderer>();
                    meshRenderer->SetMesh(mesh);
                    std::vector<std::shared_ptr<Material>> materials{};
                    for(const size_t matIndex : model->meshMaterialIndices[index]){
            ...
            auto boxModel = Model::LoadModelFromGeometry("TransparentBox", boxMesh);
            boxModel->materials[0]->SetTransparent(true);
            boxModel->materials[0]->SetBothSide(true);
            boxModel->materials[0]->SetTexture(ShaderResource::MaterialTex::kBaseColor, transparentTex);
            boxModel->materials[0]->SetTexture(ShaderResource::MaterialTex::kEmissive, transparentTex);
            boxModel->materials[0]->SetEmissiveColor({0.8, 0.8, 0.8, 1});
            ...
            auto lihuazouModel = Model::LoadModel("Assets/Models/AB/AliceADefault/AliceADefault.fbx");
            ...
            auto sponzaModel = Model::LoadModel("Assets/Models/Sponza/pbr/sponza2.gltf");
