...
namespace DSM::D3D12{
    ...
    bool Device::BindBufferMemory(IBuffer *_buffer, IHeap *_heap, uint64_t offset)
    {
        Buffer* buffer = Utility::CheckedCast<Buffer*>(_buffer);
        Heap* heap = Utility::CheckedCast<Heap*>(_heap);
        if(buffer == nullptr || heap == nullptr){
            m_Context.Error("BindBufferMemory failed: buffer or heap is null.");
            return false;
        }
    ...
    {
        auto pipeline = new RayTracingPipeline(this);
        if(pipeline == nullptr || !pipeline->Create(desc)){
            m_Context.Error("Failed to create a ray tracing pipeline.");
            return nullptr;
        }
